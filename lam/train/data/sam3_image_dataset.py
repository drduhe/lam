...
from lam.model.box_ops import box_xywh_to_xyxy
        ...
        for current_meta in self.coco.loadImagesFromDatapoint(datapoint_id):
            ...
            try:
                if ".mp4" in path and path[-4:] == ".mp4":
                    # Going to load a video frame (lazy import avoids OpenCV/decord init conflicts)
                    try:
                        from decord import VideoReader, cpu
                    except ImportError as e:
                        raise ImportError(
                            "decord is required for video-frame loading from .mp4 paths but is not installed. "
