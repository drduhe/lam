"""``lam.train.data.collator`` — training utilities for LAM.
...
from lam.model.data_misc import (
    BatchedDatapoint,
    BatchedFindTarget,
...
def pad_tensor_list_to_longest(tensors: List[torch.Tensor], dim=0, pad_val=0) -> List[torch.Tensor]:
...
def collate_fn_api(
    ...
    for i, data in enumerate(batch):
        ...
        for j, q in enumerate(data.find_queries):
            ...
            for f in fields(q.inference_metadata):
                getattr(find_metadatas[stage_id], f.name).append(getattr(q.inference_metadata, f.name))
            ...
            if q.input_points is not None:
                stages[stage_id].input_points.append(q.input_points.squeeze(0))  # Strip a trivial batch index
            ...
            if with_seg_masks:
                ...
                for object_id in q.object_ids_output:
                    ...
                    if seg_mask is not None:
                        ...
                        dummy_mask = torch.zeros(data.images[q.image_id].data.shape[-2:], dtype=torch.bool)
    ...
    for i in range(len(stages)):
        stages[i].input_points = pad_tensor_list_to_longest(stages[i].input_points, dim=0, pad_val=0)
        # Masked-out regions indicated by 1s.
        stages[i].input_points_mask = pad_tensor_list_to_longest(stages[i].input_points_mask, dim=0, pad_val=1)
    ...
    for i in range(len(stages)):
        ...
        stages[i].input_boxes_label = pad_tensor_list_to_longest(stages[i].input_boxes_label, dim=0, pad_val=0)
        # Masked-out regions indicated by 1s.
        stages[i].input_boxes_mask = pad_tensor_list_to_longest(stages[i].input_boxes_mask, dim=0, pad_val=1)
