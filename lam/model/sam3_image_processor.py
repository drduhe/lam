"""High-level image inference API: ``set_image`` / ``set_text_prompt`` for LAM."""
...
from lam.model.data_misc import FindStage, interpolate
from lam.model.model_misc import get_default_device
from lam.perflib.masks_ops import mask_iou


def _functional_to_image(image):
    """Normalize inputs to CHW float in [0, 1]; supports older torchvision without v2.functional.to_image."""
    fn = getattr(v2.functional, "to_image", None)
    if fn is not None:
        return fn(image)
    if isinstance(image, PIL.Image.Image):
        return v2.ConvertImageDtype(torch.float32)(v2.functional.pil_to_tensor(image))
    if isinstance(image, np.ndarray):
        t = torch.from_numpy(np.ascontiguousarray(image))
        return _tensor_to_chw_float01(t)
    if isinstance(image, torch.Tensor):
        return _tensor_to_chw_float01(image)
    raise TypeError(f"Unsupported image type: {type(image)}")


def _tensor_to_chw_float01(t: torch.Tensor) -> torch.Tensor:
    if t.ndim == 2:
    ...
    if t.dtype == torch.uint8:
        return v2.ConvertImageDtype(torch.float32)(t.contiguous())
...
class Sam3Processor:
    ...
    def __init__(
        self, model, resolution=1008, device=None, confidence_threshold=0.5, fuse_detections_iou_threshold=None
    ):
        """
        Args:
            model: The SAM3 model
            resolution: Image resolution for processing
            device: Device to run on ('cuda', 'mps', or 'cpu'). If None, automatically selects the best available device.
    ...
    def set_image(self, image, state=None):
        ...
        image = _functional_to_image(image).to(self.device)
    ...
    def set_image_batch(self, images: List[PIL.Image.Image], state=None):
        """Sets the image batch on which we want to do predictions (PIL images only)."""
        if state is None:
            state = {}

        if not isinstance(images, list):
            raise ValueError("Images must be a list of PIL images")
        ...
        images = [self.transform(_functional_to_image(image).to(self.device)) for image in images]
        ...
        for group_indices in groups.values():
            if len(group_indices) == 1:
                ...
                merged_mask_logits = torch.where(
                    merged_mask_binary, torch.clamp(merged_mask_logits, min=0.5), merged_mask_logits
                )
