"""Mask NMS: CUDA generic NMS, Triton fallback, or CPU reference."""
...
def nms_masks(
    ...
    """Score-threshold detections then suppress overlapping masks by mask IoU.

    Args:
        pred_probs: ``(num_det,)`` scores.
        pred_masks: ``(num_det, H, W)`` soft or logits; binarized at 0 for IoU.
        prob_threshold: Minimum score to enter NMS.
        iou_threshold: Pairwise mask IoU above which one box is suppressed.

    Returns:
        Boolean mask ``(num_det,)`` — ``True`` where a detection is kept (after threshold + NMS).
    """
...
def generic_nms(ious: torch.Tensor, scores: torch.Tensor, iou_threshold=0.5) -> torch.Tensor:
    """NMS from a full pairwise IoU matrix (CUDA / Triton / CPU).

    Args:
        ious: ``(N, N)`` symmetric IoU matrix.
        scores: ``(N,)`` per-candidate scores.
        iou_threshold: Suppression threshold.

    Returns:
        Long tensor of indices to keep.
    """
    ...
    if ious.is_cuda:
        if GENERIC_NMS_AVAILABLE:
            ...
            from lam.perflib.triton.nms import nms_triton
...
def generic_nms_cpu(ious: torch.Tensor, scores: torch.Tensor, iou_threshold=0.5) -> torch.Tensor:
    """Greedy NMS on CPU using a precomputed IoU matrix.
    ...
    Returns:
        Indices of kept boxes, sorted by descending score order in the greedy pass.
    """
