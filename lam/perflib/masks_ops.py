"""Mask IoU and axis-aligned boxes-from-masks (vectorized)."""
...
def masks_to_boxes(masks: torch.Tensor, obj_ids: list[int]) -> torch.Tensor:
    """Compute xyxy boxes tightly enclosing each binary mask plane.

    Args:
        masks: ``(N, H, W)`` tensor, one instance per plane.
        obj_ids: Length-``N`` list (callsite contract / sanity check).

    Returns:
        ``(N, 4)`` float tensor ``xmin, ymin, xmax, ymax`` on ``masks.device``.
    """
...
def mask_iou(pred_masks: torch.Tensor, gt_masks: torch.Tensor) -> torch.Tensor:
    """Pairwise IoU between two boolean mask batches (same spatial size).
