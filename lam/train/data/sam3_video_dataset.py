"""``lam.train.data.sam3_video_dataset`` — training utilities for LAM.
...
class VideoGroundingDataset(Sam3ImageDataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.tile_img_keep_find_queries_with_geo_inputs = tile_img_keep_find_queries_with_geo_inputs
    ...
    def _load_datapoint(self, index: int) -> Datapoint:
        ...
        if self.training and not self.is_tiling_single_image:
            ...
            stage_ids_to_keep = self._sample_stage_ids(queries, self.num_stages_sample, stage_stride)
    ...
    def _filter_query_and_anns(self, queries, annotations, stage_ids_to_keep, remap_stage_id, reverse_time_axis):
    ...
    def _subsample_queries(self, datapoint: Datapoint, max_query_num: int):
        ...
        sampled_find_queries_per_stage = [[queries[idx] for idx in sampled_inds] for queries in find_queries_per_stage]
