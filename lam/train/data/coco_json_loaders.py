"""``lam.train.data.coco_json_loaders`` — training utilities for LAM.
...
from lam.train.data.serialization_utils import TorchSerializedList
...
def load_coco_and_group_by_image(
    json_path: str, grouped_serialization: bool = False
    ...
    Args:
        ...
        grouped_serialization: If True, wrap the grouped list in ``TorchSerializedList``
            to reduce copy-on-read overhead with multiprocessing DataLoaders.
...
class COCO_FROM_JSON:
    ...
    def __init__(
        ...
        grouped_serialization=False,
        grouped_serialzation=None,
    ):
        """
        ...
        Args:
            ...
            grouped_serialization: Pass through to ``load_coco_and_group_by_image`` for MP DataLoader RAM use.
            grouped_serialzation: Deprecated alias for ``grouped_serialization`` (upstream typo).
        """
        if grouped_serialzation is not None:
            grouped_serialization = bool(grouped_serialzation)
        self._raw_data, self._cat_idx_to_text = load_coco_and_group_by_image(
            annotation_file,
            grouped_serialization=grouped_serialization,
        )
...
class SAM3_VEVAL_API_FROM_JSON_NP:
    ...
    def __init__(self, annotation_file):
        ...
        for video_np_dict in data["video_np_pairs"]:
            self._video_id_to_np_ids[video_np_dict["video_id"]].append(video_np_dict["category_id"])
            assert (
