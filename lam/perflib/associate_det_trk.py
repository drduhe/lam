"""Hungarian-style detection–track association from mask IoU (optional / legacy)."""
...
def associate_det_trk(
    ...
    """Associate detection masks to track masks via IoU and Hungarian assignment.

    Args:
        det_masks: ``(N, H, W)`` detection masks.
        track_masks: ``(M, H, W)`` track masks (resized to match if shapes differ).
        iou_threshold: IoU floor for a detection–track pair to count as matched.
        iou_threshold_trk: Stricter threshold for marking a track as matched.
        det_scores: Optional ``(N,)`` scores (used with ``new_det_thresh``).
        new_det_thresh: Minimum score for a detection to be emitted as ``new`` when unmatched.

    Returns:
        Tuple of lists and dicts describing new detections, unmatched tracks, and match
        bookkeeping (see implementation for exact structure).
    """
