"""Axial rotary position embeddings for 2D feature maps and attention.

Adapted from CodeLlama / RoPE-ViT / lucidrains implementations (see inline history).
...
def init_t_xy(end_x: int, end_y: int, scale: float = 1.0, offset: int = 0, device=None):
    """Flattened (x, y) coordinate grids for a ``end_x * end_y`` map."""
...
def compute_axial_cis(
    ...
    """Complex rotary frequencies for separate x and y axes, concatenated."""
...
def reshape_for_broadcast(freqs_cis: torch.Tensor, x: torch.Tensor):
    """View ``freqs_cis`` so it broadcasts over trailing spatial dims of ``x``."""
    ndim = x.ndim
    assert 0 <= 1 < ndim
...
def apply_rotary_enc(
    ...
    """Apply complex rotary encoding to query/key pairs (last dim paired as complex)."""
...
def complex_mult(xq_real, xq_imag, freqs_cis_real, freqs_cis_imag):
    """Multiply complex numbers given separate real/imag tensors."""
...
def apply_rotary_enc_real(
    ...
    """Rotary encoding using explicit real/imag frequency tensors (non-complex dtype path)."""
...
def broadcat(tensors, dim=-1):
    """Concatenate tensors after broadcasting them to a common shape."""
    broadcasted_tensors = broadcast_tensors(*tensors)
    return torch.cat(broadcasted_tensors, dim=dim)


def rotate_half(x: torch.Tensor):
    """Swap and negate half-channels (standard RoPE helper on the last dimension)."""
...
class VisionRotaryEmbeddingVE(nn.Module):
    """2D rotary embedding table for vision encoders (cos/sin buffers)."""
