...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_ffn(self, tgt):
        amp_dt = tgt.device.type if tgt.device.type in ("cuda", "cpu", "mps") else "cpu"
        with torch.amp.autocast(device_type=amp_dt, enabled=False):
    ...
    def __init__(
        ...
        if boxRPB != "none":
            ...
            if resolution is not None and stride is not None:
                ...
                coords_h, coords_w = self._get_coords(feat_size, feat_size, device=device)
