...
from lam.model.data_misc import BatchedInferenceMetadata, interpolate
from lam.train.masks_ops import rle_encode, robust_rle_encode
...
class PostProcessImage(nn.Module):
    ...
    def __init__(
        ...
        always_interpolate_masks_on_gpu: bool = torch.cuda.is_available(),
    ...
    def _process_masks(self, target_sizes, pred_masks, consistent=True, keep=None):
        ...
        if self.always_interpolate_masks_on_gpu:
            ...
            if device.type == "cpu":
                logging.warning(
                    "always_interpolate_masks_on_gpu=True but data is on CPU; falling back to CPU interpolation"
                )
